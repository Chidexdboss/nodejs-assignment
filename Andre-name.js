...
const router = async function (req, res) {
  if (req.url === "/api/my_name" && req.method === "GET") {
    // set the status code, and content-type. Status code and Content type  set
    res.writeHead(200, { "Content-Type": "application/json" });
    // send the data. To send the data 
    res.end(JSON.stringify(arr));
  }

  //  POST: /api/name Name of the API
  if (req.url === "/api/save_name" && req.method === "POST") {
    try {
      ...
      readline.question(
        "What name would you like to add into the array?",
        (new_name) => {
          console.log(` ${new_name} You has been successfully added to the array!`);
          arr.push(new_name);
          readline.close();
        }
      );
